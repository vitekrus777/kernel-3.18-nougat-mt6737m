...
static void battery_update(struct battery_data *bat_data)
{
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		...
		bat_data->BAT_current_max = battery_meter_get_charging_current() * 1000;
		bat_data->BAT_voltage_max = battery_meter_get_charger_voltage() * 1000;
		...
		bat_data->BAT_current_max = 0;
		bat_data->BAT_voltage_max = 0;
	}
...
// opium for recovery charging*/
		...
		{
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] recovery charging after over voltage !! \r\n");
...
{
	ktime_t ktime;
	unsigned int hv_voltage = 10500*1000;  //opium batt_cust_data.v_charger_max * 1000;
