...
#define MSE_FUN(f)				pr_err(MSE_TAG"%s\n", __FUNCTION__)
#define MSE_ERR(fmt, args...)	pr_err(MSE_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define MSE_LOG(fmt, args...)	pr_err(MSE_TAG fmt, ##args)
...
static struct qmcX983_i2c_data *get_obj(void)
{
	if (unlikely(this_client == NULL)) {
		MSE_ERR("this_client is null!\n");
		return NULL;
	}
	return i2c_get_clientdata(this_client);
}
...
static int I2C_RxData(char *rxData, int length)
{
	uint8_t loop_i;
    int res = 0;
#if DEBUG || (defined(Android_Marshmallow) && !defined(CONFIG_MTK_I2C_EXTENSION))
	int i;
	struct qmcX983_i2c_data *data = get_obj();
	char addr = rxData[0];
#endif
#if defined(Android_Marshmallow) && !defined(CONFIG_MTK_I2C_EXTENSION)
	...
	{
		MSE_ERR("rxdata is null or length < 1 \n");
		return -EINVAL;
	}
	...
	for(loop_i = 0; loop_i < QMCX983_RETRY_COUNT; loop_i++)
	{
		res = i2c_transfer(data->client->adapter, msg, ARRAY_SIZE(msg));
		if(res > 0)
		{
			break;
		}
...
static int I2C_TxData(char *txData, int length)
{
	...
	{
		MSE_ERR("txdata is null or length < 2 \n");
		return -EINVAL;
	}
...
static int qmcX983_read_mag_xyz(int *data)
{
	...
	unsigned char rdy = 0;
	int i = 0;
	struct qmcX983_i2c_data *clientdata = get_obj();
	

	if (unlikely(clientdata == NULL)) {
	...
	while(!(rdy & 0x07) && t1<3){
		...
		if(atomic_read(&clientdata->trace) & QMC_DATA_DEBUG)
...
int qmcX983_set_range(short range)
{
	...
	struct qmcX983_i2c_data *obj = get_obj();
	int ran ;
	switch (range) {
	case QMCX983_RNG_2G:
		ran = RNG_2G;
...
{
/*to do : sensor's power is always on, no need to control.*/
}
...
static long qmcX983_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char buff[QMCX983_BUFSIZE] = {0};				/* for chip information */
	char rwbuf[16] = {0}; 		/* for READ/WRITE */
	int value[CALIBRATION_DATA_SIZE] = {0};			/* for SET_YPR */
	int delay = 0;			/* for GET_DELAY */
	int status = 0; 			/* for OPEN/CLOSE_STATUS */
	int ret = -1;				
	short sensor_status = 0;		/* for Orientation and Msensor status */
	...
	if(clientdata == NULL){
		MSE_ERR("%s clientdata is null \n", __func__);
		return -EINVAL;
	}else if (atomic_read(&clientdata->trace) & QMC_FUN_DEBUG)
		MSE_LOG("qmcX983_unlocked_ioctl !cmd= 0x%x\n", cmd);
	
	memset(&osensor_data, 0, sizeof(osensor_data));	
...
static int qmcX983_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	{
		err = qmcX983_SetPowerMode(obj->client, false);
		if(err < 0)
		{
...
static int qmcX983_resume(struct i2c_client *client)
{
	...
	if(v_open_flag != 0)
	{
		err = qmcX983_SetPowerMode(obj->client, true);
		if(err < 0)
		{
			MSE_LOG("qmcX983: write power control fail!!\n");
			return 0;
...
static int qmcX983_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    this_client->addr = qst_hw->i2c_addr[0];
	/* read chip id */
	databuf[0] = 0x0d;
	err = I2C_RxData(databuf, 1);
	pr_err("ym_test databuf=0x%x\n",this_client->addr);	//add for test
	if(err < 0)
	{
	...
	{
		MSE_ERR("QMCX983 check ID faild!\n");
		goto exit_kfree;
	}
...
static int __init qmcX983_init(void)
{
	...
	if (!qst_hw) {
		MSE_ERR("get_mag_dts_func failed !\n");
		return -ENODEV;
	}
#else
