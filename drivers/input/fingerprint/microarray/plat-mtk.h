...
extern int fingerprint_get_gpio_info(struct platform_device *pdev);
extern int fingerprint_set_power(int cmd);
extern int fingerprint_set_reset(int cmd);
extern int fingerprint_set_spi_mode(int cmd);
extern int fingerprint_set_eint(int cmd);
...
static struct of_device_id sof_match[] = {
	{ .compatible = "mediatek,mt6737m-fingerprint", },
	{}
};
...
void plat_set_gpio(struct platform_device *plat) {
	...
	fingerprint_get_gpio_info(plat);
	fingerprint_set_power(1);
	fingerprint_set_reset(1);
	fingerprint_set_spi_mode(1);
	fingerprint_set_eint(1);
...
int plat_request_irq(unsigned int irq, irq_handler_t handler, unsigned long flags,
	...
	const char*tname = "mediatek,mt6737m-fingerprint";
