...
#if defined(CONFIG_TPD_HAVE_BUTTON)
...
#if defined(CONFIG_GTP_GESTURE_WAKEUP)
typedef enum
{
    DOZE_DISABLED = 0,
    DOZE_ENABLED = 1,
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
#if (defined(CONFIG_GTP_SLIDE_WAKEUP) || defined(CONFIG_GTP_GESTURE_WAKEUP))
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 5)
    {
        ...
        if ((!ret) && ((hw_info == 0x00900600) || (hw_info == 0x00901003)))              //20121212
        {
            return ret;
        }
...
//opium save config
ssize_t save_config_id(void)
{
	int retval;
	u16 version_info = 0;

	/* Get device config ID */	
	retval = gtp_read_version(i2c_client_point, &version_info);	
	if (retval < 0)
	{
		return retval;
	}

	tpd_firmware_version[0] = (int)(version_info);
	tpd_firmware_version[1] = (int)(config[GTP_ADDR_LENGTH]);
...
static int tpd_irq_registration(void)
{
	...
	if(node){
		...
		{
			ret = request_irq(touch_irq, tpd_eint_interrupt_handler, IRQF_TRIGGER_RISING, "TOUCH_PANEL-eint", NULL);
...
static ssize_t gt9xx_fwversion_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    ssize_t num_read_chars = 0;
    s32 ret = -1;
    s32 i;

    u8 reg[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};
    u8 config_reg[3] =  {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};

    GTP_DEBUG_FUNC();

    ret = gtp_i2c_read(i2c_client_point, reg, sizeof(reg));

    if (ret < 0)
    {
        num_read_chars = snprintf(buf, PAGE_SIZE, "Firmware version can't read!!!\n");
        return num_read_chars;
    }

    ret = gtp_i2c_read(i2c_client_point, config_reg, sizeof(config_reg));
    ...
    if (reg[5] == 0x00)
    {  
        //num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION: %c%c%c_%02x%02x\n",reg[2], reg[3], reg[4], reg[7], reg[6]);
        num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION: %c%c%c_%02x%02x_%d\n",reg[2], reg[3], reg[4], reg[7], reg[6], config_reg[2]);
    }
    else
    {
        //num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:%c%c%c%c_%02x%02x\n",reg[2], reg[3], reg[4], reg[5], reg[7], reg[6]);
        num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:%c%c%c%c_%02x%02x_%d\n",reg[2], reg[3], reg[4], reg[5], reg[7], reg[6], config_reg[2]);
    }
    return num_read_chars;
}

static DEVICE_ATTR(fwversion, 0755, gt9xx_fwversion_show, NULL);
...
void get_gt9xx_chip_id(void)
{
    ...
    if(ret<0)
    {
        GTP_INFO("Read chip id error!!!\n");
    }
    
    if (reg[5] == 0x00)
    {                  
        g_chip_id = (reg[2]&0x0F)<<8 | (reg[3]&0x0F)<<4 | (reg[4]&0x0F);
    }
    else
    {
        g_chip_id = (reg[2]&0x0F)<<12 | (reg[3]&0x0F)<<8 | (reg[4]&0x0F)<<4 | (reg[5]&0x0F);
    }
    //GTP_DEBUG("g_chip_id = 0x%x, reg[2-5] 0x%x 0x%x 0x%x 0x%x", g_chip_id, reg[2],reg[3],reg[4],reg[5]);
}
...
void force_reset_guitar(void)
{
    ...
    GTP_INFO("opium force_reset_guitar");
	is_reseting = 1;
#ifdef CONFIG_OF_TOUCH
	gt9xx_irq_disable();
#else
...
#if defined(CONFIG_GTP_GESTURE_WAKEUP)
//	GTP_INFO("opium gesture %s line %d\n",__func__,__LINE__);
//        printk("opium doze_status = %d, tpd_halt = %d\n",doze_status,tpd_halt);
        if (DOZE_ENABLED == doze_status && tpd_halt == 1)
        {
            if(g_chip_id == 0x9176)
                doze_buf[1] = 0x4C;

            ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
            GTP_DEBUG("0x%x%x = 0x%02X", doze_buf[0],doze_buf[1],doze_buf[2]);
            if (ret > 0)
            {
                GTP_DEBUG("opium  %s line %d doze_buf[2] %d\n",__func__,__LINE__,doze_buf[2]);
                isValuedKey = true;
                ...
                if(isValuedKey){
                    ...
                    ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
                    printk("opium doze_buf[2] = %d\n",doze_buf[2]);
                    if(ret > 0)
                    {
                        gpointnum = doze_buf[2];
                        GTP_DEBUG("opium %s line %d gpointnum %d\n",__func__,__LINE__,gpointnum);
                        for(i=0; i< gpointnum; i++)
                        {
                            coor_reg = GTP_REG_GES_COOR + i*4;
                            ges_data[0] = (u8)(coor_reg>>8);
                            ges_data[1] = (u8)(coor_reg&0xFF);
                            ret = gtp_i2c_read(i2c_client_point, ges_data, 6);
                            GTP_DEBUG("opium %s line %d ges_data 0x%x 0x%x, 0x%x 0x%x, 0x%x 0x%x\n",
                                    __func__,__LINE__,ges_data[0],ges_data[1],ges_data[3],ges_data[2],ges_data[5],ges_data[4]);
                            if(ret > 0)
                            {
                                coordinate_x[i] = ges_data[3]<<8 | ges_data[2];
                                coordinate_y[i] = ges_data[5]<<8 | ges_data[4];
                            }
                        }
                    }
        ...
        if (tpd_dts_data.use_tpd_button)
        {
            ...
            if (key_value || pre_key)
            {
                for (i = 0; i < TPD_KEY_COUNT; i++)
                {
                    ...
                    {
                        input_x = tpd_dts_data.tpd_key_dim_local[i].key_x;
                        input_y = tpd_dts_data.tpd_key_dim_local[i].key_y;
...
static s8 gtp_enter_doze(struct i2c_client *client)
{
    ...
    GTP_DEBUG("opium entering doze mode...");
    while(retry++ < 5)
    {
        ...
        if (ret < 0)
        {
            GTP_DEBUG("opium failed to set doze flag into 0x8046, %d", retry);
            continue;
        }
        i2c_control_buf[0] = 0x80;
        ...
        if (ret > 0)
        {
            ...
            GTP_DEBUG("GTP has been working in doze mode! int_type %d irq_flag %d\n",int_type,irq_flag);
            return ret;
        }
        msleep(10);
    }
    GTP_ERROR("opium GTP send doze cmd failed.");
    return ret;
}
...
static void tpd_suspend(struct early_suspend *h)
{
...
#else
#if defined(CONFIG_GTP_GESTURE_WAKEUP)
    printk("opium gesture en doze,_is_open_gesture_mode = %d\n",_is_open_gesture_mode);
    if(_is_open_gesture_mode)
    {
        GTP_INFO("opium enter gesture%s line %d\n",__func__,__LINE__);
        ret = gtp_enter_doze(i2c_client_point);
        printk("opium ret = %d\n",ret);
	}
    else
	...
	if (ret < 0)
	{
		GTP_ERROR("opium GTP early suspend failed.");
	}	
#endif
